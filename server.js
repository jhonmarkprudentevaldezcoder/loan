...
const connectionString =
  "mongodb+srv://akosijaycee:Eyeshield232045@cluster0.rb77oza.mongodb.net/";
...
app.get("/getClosedLoans", async (req, res) => {
  try {
    ...
    const closedLoans = await usersLoanCollection
      .find({ username, status: "Closed" })
      .toArray();
...
app.get("/getRejectedLoans", async (req, res) => {
  try {
    ...
    // Fetch closed loans based on a specific status (e.g., 'Closed')
    const rejectedLoans = await usersLoanCollection
      .find({ username, status: "Rejected" })
      .toArray();
